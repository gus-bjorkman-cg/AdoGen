...
public class SqlServerBulkTests
{
    ...
    public void DomainFile_ShouldBeGenerated() =>
        Source.RunGenerator(Provider).Result.GetGeneratedText(DomainFileName).Should().NotBeEmpty();
    
    [Fact]
    public Task BulkFile_ShouldMatchSnapshot() =>
        Verify(Source.RunGenerator(Provider).Result.GetGeneratedText(BulkFileName));
