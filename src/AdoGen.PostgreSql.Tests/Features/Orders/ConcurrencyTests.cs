...
namespace AdoGen.PostgreSql.Tests.Features.Orders;
...
public sealed class ConcurrencyTests(TestContext testContext) : TestBase(testContext)
{
    ...
    public async Task Update_ShouldBumpVersion()
    {
        ...
        var version = await Connection.QueryScalarFirstOrDefaultAsync<int>(
            """SELECT "Version" FROM "public"."Orders" WHERE "Id" = @Id""",
            OrderNpgsql.CreateParameterId(DefaultOrders[0].Id),
            CancellationToken);
    ...
    public async Task Update_ShouldThrowConcurrencyException_WhenStaleVersion()
    {
        ...
        // Assert
        (await act.Should().ThrowAsync<AdoGenConcurrencyException>()).Which.TableName.Should().Be("public.Orders");
    }
    ...
    public async Task Update_ShouldThrowConcurrencyException_WhenStaleVersionAndConcurrencyIsGuid()
    {
        ...
        (await act.Should().ThrowAsync<AdoGenConcurrencyException>()).Which.TableName.Should().Be("public.Order2s");
    }
