...
public sealed class PatchUserNpgsqlCommandHandlerTests(TestContext testContext) : TestBase(testContext)
{
    ...
    public async Task PatchUser_ShouldUpdateOnlySetColumns()
    {
        ...
        // Act
        await _sut.NpgSql(new PatchUserNpgsqlCommand(patch), CancellationToken);
    ...
    public async Task PatchUser_ShouldReturnZero_WhenNoColumnsSet()
    {
        ...
        // Act
        var affected = await _sut.NpgSql(new PatchUserNpgsqlCommand(patch), CancellationToken);
