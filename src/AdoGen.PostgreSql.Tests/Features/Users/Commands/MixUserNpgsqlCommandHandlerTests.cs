...
public sealed class MixUserNpgsqlCommandHandlerTests(TestContext testContext) : TestBase(testContext)
{
    ...
    public async Task MixUserCommandHandler_ShouldMixUsers()
    {
        ...
        // Act
        await _sut.NpgSql(new MixUserCommand(usersToInsert, usersToUpdate, usersToDelete), CancellationToken);
