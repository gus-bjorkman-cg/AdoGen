...
public sealed class UpdateUserNpgsqlCommandHandlerTests(TestContext testContext) : TestBase(testContext)
{
    ...
    public async Task User_ShouldBeUpdated()
    {
        ...
        // Act
        await _sut.NpgSql(new UpdateUserCommand(user), CancellationToken);
