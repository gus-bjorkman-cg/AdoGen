    ...
    private static void EmitWithInfo(SourceProductionContext spc, INamedTypeSymbol dto, ProfileInfo info)
    {
        ...
        var src = $$""""
            ...
            public sealed partial {{typeKeyword}} {{dto.Name}} : ISqlDomainModel<{{dtoTypeName}}>
            {
                ...
                public static async ValueTask CreateTableAsync(SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                ...
                public static async ValueTask InsertAsync({{dtoTypeName}} model, SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    if (connection.State != ConnectionState.Open) await connection.OpenAsync(ct).ConfigureAwait(false);
            ...
            {{ParamAdd("model")}}
                ...
                public static async ValueTask InsertAsync(List<{{dtoTypeName}}> models, SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    ...
                    await using var cmd = connection.CreateCommand(sb.ToString(), CommandType.Text, transaction);
            ...
            {{ParamAddBatchFlat("model", "paramIndex")}}
                ...
                public static async ValueTask UpdateAsync({{dtoTypeName}} model, SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    if (connection.State != ConnectionState.Open) await connection.OpenAsync(ct).ConfigureAwait(false);
            ...
            {{ParamAddForUpdate("model")}}
                ...
                public static async ValueTask DeleteAsync({{dtoTypeName}} model, SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    if (connection.State != ConnectionState.Open) await connection.OpenAsync(ct).ConfigureAwait(false);
            ...
            {{ParamAddForDelete("model")}}
                ...
                public static async ValueTask UpsertAsync({{dtoTypeName}} model, SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    if (connection.State != ConnectionState.Open) await connection.OpenAsync(ct).ConfigureAwait(false);
            ...
            {{ParamAdd("model")}}
                ...
                public static async ValueTask TruncateAsync(SqlConnection connection, CancellationToken ct, SqlTransaction? transaction = null)
                {
                    if (connection.State != ConnectionState.Open) await connection.OpenAsync(ct).ConfigureAwait(false);
