...
internal sealed class DomainOpsEmitterNpgSql : IEmitter
{
    ...
    public void Handle(SourceProductionContext spc, ValidatedDiscoveryDto validatedDto, EmitContext ctx)
    {
        ...
        // INSERT (skip identity) — use pre-computed NonIdentities
        ...
        // UPSERT via ON CONFLICT — exclude identity keys from conflict target
        var nonIdentityKeys = ctx.Keys.Where(col => !col.IsIdentity).ToArray();
        var conflictKeys = BuildJoined(ImmutableArray.Create(nonIdentityKeys), col => col.ColumnNameQuoted);
        var updateSetOnConflict = BuildJoined(ctx.NonKeyNonIdentities, col =>
            $"""{col.ColumnNameQuoted} = EXCLUDED.{col.ColumnNameQuoted} """);

        var upsertSql =
            $"{insertSql.TrimEnd(';')} ON CONFLICT ({conflictKeys}) DO UPDATE SET {updateSetOnConflict};";
